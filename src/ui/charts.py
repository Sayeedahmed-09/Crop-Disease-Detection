...
Visualization components for AgroSentry.
...
def render_confidence_indicator(
    ...
    if confidence >= 0.90:
        st.success(f"High Confidence ({percentage:.2f}%)")
    elif confidence >= 0.70:
        st.warning(f"Medium Confidence ({percentage:.2f}%)")
    else:
        st.error(f"Low Confidence ({percentage:.2f}%)")
...
def render_probability_chart(
    ...
    if not predictions:
        st.info("Prediction probabilities are unavailable.")
        return

    dataframe = pd.DataFrame(
        {
            "Disease": [p["display_name"] for p in predictions],
    ...
    st.bar_chart(dataframe.set_index("Disease"))
